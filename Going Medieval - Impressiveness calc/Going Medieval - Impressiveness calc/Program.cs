    ...
    internal class Program
    {
        ...
        public static void Main(string[] args)
        {
            for (int i = 0; i < 100; i++)
            {
                ...
                Console.WriteLine("Your current tier is: " + FinalResult() + " / " + TierResult());
                Console.WriteLine();
            }
            Console.ReadKey();
        }

        public static float CalcLerp(float baseValue, float[] valueArray, float[] multArray)
        {
        ...
        public static TierList TierResult()
        {
            return Enum.GetValues(typeof(TierList))
                   .Cast<TierList>()
                   .Where(t => (int)t <= finalResult)
